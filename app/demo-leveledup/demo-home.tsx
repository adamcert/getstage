...
export function DemoHome({
  ...
  const { t: tdh } = useTranslation("demoHome");
  const { t: tcat } = useTranslation("categories");
  const { t: tc } = useTranslation("common");
  // Localized categories
  const localDemoCategories = [
    { id: "concert", label: tcat("concerts"), icon: Music, color: "from-primary-500 to-rose-600" },
    { id: "dj", label: tdh("clubsDj"), icon: Disc3, color: "from-secondary-500 to-purple-600" },
    { id: "theatre", label: tcat("theatre"), icon: Drama, color: "from-fuchsia-500 to-pink-600" },
    { id: "comedy", label: tcat("comedy"), icon: Laugh, color: "from-pink-500 to-rose-600" },
    { id: "festival", label: tcat("festivals"), icon: Tent, color: "from-emerald-500 to-green-600" },
    { id: "sport", label: tcat("sport"), icon: Trophy, color: "from-rose-500 to-pink-600" },
    { id: "expo", label: tcat("expos"), icon: Frame, color: "from-indigo-500 to-blue-600" },
    { id: "film", label: tcat("cinema"), icon: Film, color: "from-cyan-500 to-sky-600" },
  ];
  ...
  const spotlightTitle =
    tonightEvents.length > 0 ? tdh("tonightTitle") : tdh("dontMiss");
  const spotlightSub =
...
function PosterCard({ event }: { event: Event }) {
  ...
  const priceDisplay = minPrice === 0 ? tc("free") : `${tc("from")} ${formatPrice(minPrice)}`;
